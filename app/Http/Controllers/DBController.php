...
class DBController extends Controller
{
    ...
    public function addItems()
    {
        ...
        //$items = request()->input();
        $validator = Validator::make(request()->input(), [
            'upc' => ['required', 'regex:/\d{12}/', 'size:12', new UPC, 'bail'],
        ...
        /* $items["upc"] = $validated["upc"];
        $items["category"] = $validated["category"];
        $items["description"] = $validated["description"];
        $items["quantity"] = $validated["quantity"]; */

        if ($this->checkIfItemExists($items["upc"], $items["user_id"])) {
            return response()->json(["errors" => "UPC already exists."]);
        } else {
